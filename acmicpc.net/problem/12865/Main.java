...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[] maxValueByWeightSum = new int[k + 1];
        for (int i = 0; i < n; i++) {
            int w = io.nextInt();
            int v = io.nextInt();
            for (int weightSum = k - w; weightSum >= 0; weightSum--) {
                if (weightSum == 0 || maxValueByWeightSum[weightSum] != 0) {
                    maxValueByWeightSum[weightSum + w] = Math.max(maxValueByWeightSum[weightSum + w], maxValueByWeightSum[weightSum] + v);
                }
            }
        }
        int maxValue = 0;
        for (int weightSum = 0; weightSum <= k; weightSum++) {
            maxValue = Math.max(maxValue, maxValueByWeightSum[weightSum]);
        }
        io.print(maxValue);
